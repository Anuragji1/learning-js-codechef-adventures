**The things i did find new here!**

Statement 5.2) In every project we do, we will be learning some new things! In the last project of todo list, we cover a whole lot of basics. Now, it's time to upgrade ourselves!
...
By the end of this course, you'll have a fully functional Wordle game that demonstrates various JavaScript and jQuery concepts, as well as API integration. It's a fun way to enhance your JavaScript skills and create an interactive game to challenge your friends! Happy coding!     index.js= $(document).ready(function () {
 let secretWord = "";
 let maxTries = "";
...
-----------------------------------------------------------------------------------------------Here's a summary of the new JavaScript concepts i did encounter in this Wordle game:

1. jQuery:

Purpose: Simplifies DOM manipulation and event handling, making it easier to interact with HTML elements.
Key Features:
Uses CSS-like selectors to target elements (<span class="math-inline">\("\#element\-id"\)\). - Offers convenient methods to modify elements (`.text()`, `.addClass()`, `.append()`, etc.). - Handles events efficiently (`.on("click", function() { ... })`). - **Example in the game:** Selecting the guess input field (`("#guess-input")), appending feedback elements (.append()), and handling form submission (.on("submit", function() { ... })`).
2. forEach Loop:

Purpose: Iterates through each element of an array, executing a provided function for each item.
Syntax: array.forEach(function(element, index) { ... })
Example in the game: Rendering word containers (renderWordContainers()) and feedback history (renderHistory()).
3. Maps:

Purpose: Store key-value pairs, similar to objects, but with unique keys and more flexible ordering.
Key Features:
Uses set() and get() methods to store and retrieve values.
Maintains key order.
Example in the game: Tracking letter counts in checkWordle() to determine feedback.
4. Switch Statements:

Purpose: Conditionally execute code based on different cases.
Syntax: switch (expression) { case value1: ... break; case value2: ... break; default: ... }
Example in the game: Applying styles to letters based on feedback (showFeedback()).
5. Sounds:

Purpose: Enhance user experience with audio cues.
Key Features:
Use the <audio> element and its play() method to play sounds.
Example in the game: Playing start, success, and failure sounds (fetchRandomWord(), showResult()).
6. API Calls:

Purpose: Fetch data from external services.
Key Features:
Use the fetch() function to make API requests.
Parse responses using response.json().
Example in the game: Fetching the random word from an API (fetchRandomWord()).
